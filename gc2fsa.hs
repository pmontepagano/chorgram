...
main = do progargs <- getArgs
          if L.null progargs
            then putStrLn $ usage GC2FSA
              ...
              maingc2fsa (gc2fsafunc gctxt) flags
...
maingc2fsa (gcfsaoutput, names) flags = do
      if ("" == flags!"-o")
      then putStrLn $ L.concat gcfsaoutput
      else do
            mapM_
                  (\(p,m) ->
                        writeToFile ((flags!"-o") ++ (if p=="" then "" else "_cfsm_" ++ p) ++ ".fsa") m)
                  ([("", L.concat gcfsaoutput)] ++ (L.zip names gcfsaoutput))

foreign export ccall gc2fsaexportable :: CString -> CString
gc2fsaexportable x = L.concat (fst (gc2fsafunc x))