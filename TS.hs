...
apply fifo ptps c@(n, b) t@(_, (ch@(s,r), d, msg), q)
    ...
    | d == LoopSnd   = (toKEvent c ptps t, (Misc.update (getId s (M.assocs ptps)) q n, M.insert ch ((b!ch)++[msg]) b))
    | d == LoopRcv   = (toKEvent c ptps t, (Misc.update (getId r (M.assocs ptps)) q n, M.insert ch newBuffer b))
